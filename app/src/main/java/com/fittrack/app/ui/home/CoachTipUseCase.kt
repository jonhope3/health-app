...
class CoachTipUseCase(
                ...
                withContext(Dispatchers.IO) {
                        ...
                        val weeklySummary: String? =
                                if (includeWeeklyData) {
                                        ...
                                        val avgCal =
                                                if (calHistory.isNotEmpty())
                                                        calHistory.average().toInt()
                                                else promptData.caloriesEaten
                                        val avgSteps =
                                                if (stepsHistory.isNotEmpty())
                                                        stepsHistory.average().toInt()
                        ...
                        try {
                                if (mediaPipeLLMService.initialize()) {
                                        val response = mediaPipeLLMService.generateContent(prompt)
                                        val cleaned =
                                                response.trim('"')
                                                        .replace(Regex("""\*{3,}"""), "**")
                                                        .replace(Regex(""" {2,}"""), " ")
                                ...
                                Log.w(
                                        "FitTrack_Coach",
                                        "MediaPipe Gemma failed, trying fallback: ${e.message}"
                                )
                        }
                        ...
                        try {
                                if (geminiNanoService.initIfNeeded()) {
                                        ...
                                        if (response.isNotBlank()) {
                                                Log.d(
                                                        "FitTrack_Coach",
                                                        "Gemini Nano fallback tip generated"
                                                )
                                                return@withContext response
                                        }
                                }
                        } catch (e: Exception) {
                                Log.w("FitTrack_Coach", "Gemini Nano fallback failed: ${e.message}")
                        }
        ...
        private fun buildCoachPrompt(data: CoachPromptData, weeklySummary: String?): String {
                ...
                return buildString {
                        ...
                        append(
                                "The user's name is ${data.nickname}. It is currently $exactTime ($timeOfDay). "
                        )
                        append("Here is their data for today:\n")
                        append(
                                "- Calories eaten: ${data.caloriesEaten} of ${data.calorieGoal} cal goal\n"
                        )
                        append(

...
class CyclePredictionEngine @Inject constructor() {
    ...
    fun analyzeCycleHistory(cycles: List<CycleRecordEntity>): CycleStats? {
        ...
        if (shortest < 21) anomalies.add("Short cycle detected (${shortest}d) — under 21 days")
        if (longest > 35) anomalies.add("Long cycle detected (${longest}d) — over 35 days")
        val periodOutliers = periodLengths.filter { it > 7 }
        if (periodOutliers.isNotEmpty()) anomalies.add("Heavy/long periods (${periodOutliers.max()}d)")

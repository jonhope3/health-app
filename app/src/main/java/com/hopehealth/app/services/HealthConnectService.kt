...
class HealthConnectService {
    ...
    fun isAvailable(context: Context): Boolean {
        ...
        Log.d("HopeHealth_HC", "isAvailable check: status=$status, SDK_AVAILABLE=${HealthConnectClient.SDK_AVAILABLE}")
        return status == HealthConnectClient.SDK_AVAILABLE
    }

    suspend fun initialize(context: Context): Boolean = withContext(Dispatchers.IO) {
        try {
            Log.d("HopeHealth_HC", "initialize() called")
            if (!isAvailable(context)) {
                Log.d("HopeHealth_HC", "initialize failed: not available")
                return@withContext false
            }

            val healthClient = HealthConnectClient.getOrCreate(context)
            client = healthClient
            Log.d("HopeHealth_HC", "healthClient created")

            val granted = healthClient.permissionController.getGrantedPermissions()
            Log.d("HopeHealth_HC", "granted permissions: $granted")
            Log.d("HopeHealth_HC", "required permissions: $requiredPermissions")
            
            val isAllGranted = granted.containsAll(requiredPermissions)
            Log.d("HopeHealth_HC", "isAllGranted: $isAllGranted")
            
            isAllGranted
        } catch (e: Exception) {
            Log.e("HopeHealth_HC", "initialize exception: ${e.message}", e)
            false
        }
    }
    ...
    suspend fun readTemperatureHistory(
        ...
        try {
            ...
            Log.d("HopeHealth_HC", "BBT read failed: ${e.message}")
        }
        ...
        try {
            ...
            Log.d("HopeHealth_HC", "Body temp read failed: ${e.message}")
        }
